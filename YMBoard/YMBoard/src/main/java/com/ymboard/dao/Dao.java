...
public class Dao {
	...
	public Dao() {
		...
		try {
			...
			dataSource = (DataSource) context.lookup("java:comp/env/jdbc/eclipse");
	...
	public ArrayList<Dto> list(int requestPage, int numOfTuplePerPage) {
		...
		String query = "SELECT no, writer, title, date FROM paging ORDER BY no DESC LIMIT ?, ?";
	...
	public void write(String title, String writer, String content, String filePath) {
		...
		String query = "INSERT INTO paging (title, writer, content, date, filePath) VALUES (?, ?, ?, now(), ?)";
