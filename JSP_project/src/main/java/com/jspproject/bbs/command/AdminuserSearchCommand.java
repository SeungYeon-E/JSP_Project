...
import com.jspproject.bbs.dto.AdminUserdto;
...
public class AdminuserSearchCommand implements Command {
	...
	// 페이지당 표시할 게시글의 수
		...
		public void execute(HttpServletRequest request, HttpServletResponse response, HttpSession session) {
			...
			// 반환되는 총 튜플의 수
			int countedTuple = dao.selectUserCnt2(searching);
			System.out.println(countedTuple);
			// 페이지 목록 (1...n)
			ArrayList<Integer> pageList = calcNumOfPage(countedTuple);
			// 페이지 목록을 세션에 담는다. *list에 진입하면 무조건 세션이 갱신되므로 새 글이 생겨도 최신화가 된다.
			session.setAttribute("pageList", pageList);
			// 해당 페이지에 알맞은 번호의 게시글
			ArrayList<AdminUserdto> dtos = dao.userSelect2(searching, requestPage, numOfTuplesPerPage);
