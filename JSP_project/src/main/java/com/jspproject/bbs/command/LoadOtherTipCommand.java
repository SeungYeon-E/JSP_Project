...
public class LoadOtherTipCommand implements Command {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response, HttpSession session)
		...
		ArrayList<PostDto> dtos = dao.listOther(requestPage, numOfTuplesPerPage);
		request.setAttribute("list", dtos);
		session.setAttribute("CATEGORY", "TIP");
		session.setAttribute("TITLE", "T_TITLE");
		session.setAttribute("do", "otherTipLsit.do");
	}
